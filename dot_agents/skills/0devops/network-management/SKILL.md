...
| 出口路由器 | ISP | 带宽 | 备注 |
|-----------|-----|------|------|
| 192.168.100.1 | 联通 | 300M | 公司 ImmortalWrt，直连 PPPoE |
| 192.168.11.1 | 电信 | 50M, 上行 10M | 公司私拉 50M，光猫拨号 |
| 192.168.13.1 | 移动 | 1000M | 家里 pveImmortalWrt，直连 PPPoE |
...
Internet
    │
    ├── 联通 300M — [PPPoE] 192.168.100.1 i5 小主机 (ImmortalWrt)
    │   └── SSH 端口 23333
    │        │
    │        └── 192.168.100.100 — J1900 (eduGatewayImmortalWrt)
    |            EasyTier: 10.144.100.100/8
    │            10.55.2.118 ←→ 公司大内网 10.55.2.1 (不可控)
    │                    │
    ...
    │                    │
    │                    ▼
    ├── 电信 50M — [没改桥接] 192.168.11.1 — NanoPi R2S 路由器 (ImmortalWrt)
    │                    │
    │                    ├── 192.168.11.192 — chenPC (Fedora Bluefin)
    │                    │   └── DHCP 模式 → 默认走 11.1 (电信)
    │                    │   └── GRE 模式 → 切 hp-server 网关走 100.1 (联通)
    │                    └── EasyTier: 10.144.11.1/8
    │                         │
    │                    ─ ─ ─ ─ ─ ─ ─ ─ ─  (EasyTier 子网 VPN)
    │                         │
    ├── 移动 1000M — [PPPoE] 外部公网拨号路由器 (pveImmortalWrt) 10.121.13.242
    │   192.168.13.1 (LAN / br-lan)
    │   EasyTier: 10.144.13.1/8
    │        │
    ...
    │               └── ...
    │
    ├──── (13.x ←→ 11.x 通过 EasyTier 覆盖网络 — 10.144.x.x/8，跨越物理位置)
...
### 192.168.11.0/24 — 电信 50M LAN

| 设备 | IP | 角色 |
| -------------- | -------------- | ---------------------------------------------- |
| NanoPi R2S 路由器 | 192.168.11.1 | ImmortalWrt, DHCP 服务器, OpenClash fake-IP, EasyTier 10.144.11.1/8 |
| chenPC | 192.168.11.192 | Fedora Bluefin Linux 桌面 |
| hp-server | 192.168.11.131 | Ubuntu Server (双网卡: 同时有 10.55.225.235) |

- WAN: 电信 50M
- 替代路径: → hp-server → 公司网络 → eduGateway → 100.1 (联通 300M)

### 192.168.13.0/24 — PVE 网段

| 设备 | IP | 角色 |
| -------------- | -------------- | ------------------------------------------------------- |
| pveImmortalWrt | 192.168.13.1 | ImmortalWrt, PPPoE 出口, DHCP 服务器, OpenClash fake-IP, EasyTier 10.144.13.1/8 |
...
```
11.192 → 11.131 (chenPC 静态路由: 10.0.0.0/8 via 11.131) → GRE → 100.100 → EasyTier → 13.1
```
...
| GRE 隧道 | 11.192 → hp-server → 公司网 10.55.2.1 → eduGateway → 100.1 → 联通 300M |
...
| 校园网 DIRECT | Manual | 10.55.225.235/24 | 10.55.225.1 | 223.5.5.5 | 公司内网直连 |

### DHCP 模式静态路由

三条静态路由（`nmcli con show "有线连接 DHCP"` 的 `ipv4.routes`），都是**基础设施路由**——不管 chenPC 走哪个出口，始终必需：

| 路由 | 下一跳 | 用途 | 性质 |
|------|-------|------|------|
| `172.16.69.0/30` | `192.168.11.131` (hp-server) | GRE 隧道点对点子网，隧道保活 | 双向可达 |
| `10.0.0.0/8` | `192.168.11.131` (hp-server) | EasyTier 虚拟网段回程，走 GRE → 100.100 P2P → 13.1，绕过 50M 电信瓶颈 | 回程锚点 |
| `10.55.0.0/16` | `192.168.11.1` | 公司内网备选路径，11.1 有路由到 10.55.x | 兜底 |

**设计意图：**
- `172.16.69.0/30` 和 `10.0.0.0/8 via 11.131` 放在 DHCP 配置而非 "WY UNICOM GRE 隧道" 里，因为 GRE 隧道是双向基础设施——13.x 回程无论 chenPC 用哪个出口都走这条路径。放在 GRE 配置里的话，切回 DHCP 模式时这些路由会被撤销，回程就断了。
- **核心目的**：DHCP 模式下 EasyTier 仍可用，使 `10.144.100.100`（eduGateway）能通过回程路径 P2P 直通 `192.168.11.192`。
- `10.0.0.0/8` 用 `/8` 而非更窄范围是防御性写法——未来 EasyTier proxy_network 或别的 10.x 虚拟网段回程也不会断。
...
| 目标 | 地址 | 端口 | 系统 | 备注 |
|--------|---------|------|-----|-------|
| 本地单臂路由器 | `root@192.168.11.1` | 22 | ImmortalWrt | |
| pveImmortalWrt | `root@192.168.13.1` | 22 | ImmortalWrt | |
| eduGateway | `root@192.168.100.100` | 22 | ImmortalWrt | |
| 外部公网拨号路由器 | `root@192.168.100.1` | 23333 | ImmortalWrt | |
| hp-server | `ubuntu@192.168.11.131` | 22 | Ubuntu Server | sudo 密码: 12345678 |
| PVE Ubuntu LXC / CasaOS  | `root@192.168.13.101` | 22 | Ubuntu 24.04 | 有 Docker |
| 我的 PC (chenPC) | `chen@chenpc.lan` (`192.168.11.192`) | 22 | Fedora Bluefin | |

## OpenClash 配置

所有路由器都运行 OpenClash **fake-IP 模式**，**基于规则的代理**。
