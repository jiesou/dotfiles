...
# wps.write_header_footer（action=insert_page_number_in_footer）
...
- `alignment` (number, 可选): 对齐方式（可选）。必须显式传数字；传 0 表示左对齐（不会被当成缺省）。见 [WdParagraphAlignment](../enums.md#wdparagraphalignment--对齐方式)
