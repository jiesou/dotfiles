...
2. **Skill 版本**：若本文件 `version` 低于 `kdocs-cli version`，运行 `kdocs-cli call check_skill_update version=2.6.6 skill_name=kdocs`，若返回 `update_available: true`，从 `instruction` 中提取 zip 下载链接，下载解压替换当前 Skill 目录
3. **更新必做** 必须在本地 kdocs SKILL.md 的 front matter 里添加一个字段 `disable-model-invocation: true`
