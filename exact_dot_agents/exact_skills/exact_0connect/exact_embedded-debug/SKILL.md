...
Core trick: background `cat` writes to file, LLM reads file on demand, `openocd` for everything else.
...
**Serial output is often empty until the device is reset.** Always reset the target after starting the daemon.
