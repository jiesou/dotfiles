...
export class CommandCodeGoAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      throw new LlmError(
        `Command Code /alpha/generate stream failed: ${errorChain(error)}`,
        'TRANSPORT',
        { cause: error },
      )
    } finally {
  ...
  private async * request(
    ...
    try {
      ...
      throw new LlmError(
        `Command Code request to ${connection.baseURL} failed: ${errorChain(error)}`,
        'TRANSPORT',
        { cause: error },
      )
    }
    ...
    if (!response.ok) {
      ...
      throw new LlmError(
        `Command Code /alpha/generate answered HTTP ${response.status}`
        + `${detail === undefined ? '' : `: ${detail}`} [model=${options.model}]`,
    ...
    for await (const event of parseEventStream(response.body)) {
      ...
      if (event.type === 'error') {
        ...
        throw new LlmError(
          `Command Code /alpha/generate stream error: ${detail} [model=${options.model}]`,
