...
本插件针对第二种情况：通过 `/alpha/generate` 流式接入 DSH 的原生 `LlmAdapter`，让 Go / GOAT / Pro Plan 用户直接在 DSH 中使用订阅的模型。模型列表不写死在代码里，插件会定时从 `/provider/v1/models` 拉取实时目录，并按官方 CLI catalog (CDN) 的 `Min plan` 列筛选：
...
Reasoning effort 不需要配置：档位来自官方 CLI catalog，模型只暴露它真正接受的档位（`low`/`medium`/`high`/`xhigh`/`max`）。网关没有"关闭思考"的取值，**不选档位（Default）就是不发送该字段、由模型自行决定深度**，所以插件不提供假的 `Off` 选项；catalog 里档位为空的模型干脆不显示档位选择器。
