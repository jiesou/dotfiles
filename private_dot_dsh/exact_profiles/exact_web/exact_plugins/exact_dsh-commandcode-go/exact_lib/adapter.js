 ...
 *
 * The gateway's effort vocabulary is `low` … `max` with no value that turns
 * thinking off: naming no effort IS the provider default. So a model exposes
 * exactly the efforts the catalog credits it with, and a model the catalog
 * leaves blank (it decides its own depth) exposes no selector — offering an
 * `Off` entry that merely omits the field would promise a switch the gateway
 * does not have, and offering an effort it rejects would fail the request.
 * No default effort is pinned either: absence preserves the gateway's own.
 */
function reasoningFor(model) {
    const levels = (model?.efforts ?? []).filter(effort => effort in GATEWAY_EFFORTS);
    return levels.length === 0 ? undefined : { efforts: levels.map(effortInfo) };
}
/**
...
export class CommandCodeGoAdapter extends LlmAdapter {
        ...
        try {
            ...
            try {
                ...
                if (error instanceof LlmError)
                    throw error;
                throw new LlmError(`Command Code /alpha/generate stream failed: ${errorChain(error)}`, 'TRANSPORT', { cause: error });
            }
            finally {
    ...
    async *request(options, signal, connection, apiKey) {
        ...
        try {
            ...
            throw new LlmError(`Command Code request to ${connection.baseURL} failed: ${errorChain(error)}`, 'TRANSPORT', { cause: error });
        }
        if (!response.ok) {
            const raw = await response.text().catch(() => '');
            const detail = gatewayErrorDetail(raw);
            throw new LlmError(`Command Code /alpha/generate answered HTTP ${response.status}`
                + `${detail === undefined ? '' : `: ${detail}`} [model=${options.model}]`, upstreamErrorCode(response.status, detail ?? raw), { status: response.status });
        }
        if (!response.body) {
        ...
        for await (const event of parseEventStream(response.body)) {
            ...
            if (event.type === 'error') {
                ...
                throw new LlmError(`Command Code /alpha/generate stream error: ${detail} [model=${options.model}]`, upstreamErrorCode(status ?? 500, detail), status === undefined ? undefined : { status });
            }
        ...
        throw new LlmError('Command Code stream ended without a finish event', 'STREAM_CLOSED');
    }
}
/**
