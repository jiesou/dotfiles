...
let activeSubscription: PushSubscription | undefined
...
export async function ensureSubscription(): Promise<void> {
  ...
  try {
    ...
    const subscription = existing ?? await (async () => {
      ...
      if (data.enabled !== true || typeof data.publicKey !== 'string') return undefined
...
export function reportFocus(focused: boolean): void {
  if (!pushSupported()) return
  void fetch(`${PLUGIN_PREFIX}/focus`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ focused }),
  }).catch(() => { /* focus reporting is best-effort */ })
}
...
export function installPushBridge(opts: { openSession: (sessionId: string) => void }): void {
  if (!pushSupported()) return
